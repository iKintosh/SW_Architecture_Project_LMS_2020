...
Kant_philosophy = Material(
    id=1,
    name="Kants conception of philosophy",
    text="some text about Kant",
    date=dt.datetime(2019, 11, 1, 10, 10, 00),
    course_id=philosophy_course.id,
)
Matrix_math = Material(
    id=2,
    name="Matrixes and matrix operations",
    text="some math matrix blablabla difficult",
    date=dt.datetime(2019, 11, 25, 10, 10, 00),
    course_id=math_course.id,
)
Integral_math = Material(
    id=3,
    name="Integrals",
    text="something(x) dx = F(something(x))",
    date=dt.datetime(2019, 11, 10, 10, 00, 00),
    course_id=math_course.id,
)

homework_matrix = Homework(
    id=1,
    name="Practice in matrix operations",
    valid_from=dt.datetime(2019, 11, 29, 10, 00, 00),
    valid_to=dt.datetime(2019, 12, 1, 10, 00, 00),
    description="AX=b, X=?",
    course_id=math_course.id,
)
homework1_philosophy = Homework(
    id=2,
    name="Your fist philosophical essay",
    valid_from=dt.datetime(2019, 11, 29, 10, 10, 00),
    valid_to=dt.datetime(2019, 12, 15, 19, 00, 00),
    description="Write an essay on philosophical topic",
    course_id=philosophy_course.id,
)
john_answer_philosophy = Answer(
    id=1,
    user_id=john.id,
    homework_id=homework1_philosophy.id,
    answer="John's essay",
    submit_date=dt.datetime(2019, 12, 11, 9, 00, 00),
)
alice_answer_philosophy = Answer(
    id=2,
    user_id=alice.id,
    homework_id=homework1_philosophy.id,
